...
- 增加“启用日历隐藏”开关；当前解锁动作为双击年份后长按月份。关闭后恢复 Hail 的桌面名称和默认图标。
- 应用默认名称改为“万年历”，使用新的日历图标，包名改为 `com.aistra.calendar`，APK 文件名改为 `Calendar-v版本号.apk`。
- 增加“无图标应用启动器”，可单独隐藏或恢复其他应用的桌面图标，并从管理页面启动这些应用。
- 无图标应用列表支持搜索、排序和筛选，操作过的应用优先显示。
- 将工作模式拆成“权限”和“默认模式”两行，先选授权方式，再选该权限支持的操作。
- 在“权限”和“默认模式”下增加“添加操作应用”入口，并隐藏底部导航栏中的“应用”按钮。
- 将“生物识别验证”移动到“自定义”分组最上方。
...
- 关于页面名称改为 `hail_2`，增加本分支 GitHub 入口，并保留原 Hail 项目和给原作者捐赠的入口。
- 项目版本重新从 `v1.0` 开始，内部版本号为 `1`。
...
- 关于页面中的 Telegram 群组、QQ群、F-Droid 和翻译入口。
